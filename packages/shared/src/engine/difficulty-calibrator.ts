...
export interface TechniqueAnalysis {
  ...
  frequency: number; // How often this technique is needed
  complexity: number; // Relative complexity score
  timeImpact: number; // Impact on solve time
}
...
export class DifficultyCalibrator {
  ...
  calibrateDifficulty(puzzle: Puzzle, forGeneration: boolean = false): DifficultyCalibrationResult {
  ...
  private analyzeTechniques(puzzle: Puzzle, forGeneration: boolean = false): TechniqueAnalysis[] {
    ...
    const techniqueUsage = new Map<SolvingTechnique, number>();
    ...
    while (!this.isGridComplete(workingGrid) && iterations < maxIterations) {
      ...
      if (!progress) {
        ...
        techniqueUsage.set(advancedTechnique,
          (techniqueUsage.get(advancedTechnique) || 0) + 1);
        break;
      }
    }
  ...
  private calculateDifficultyScore(techniques: TechniqueAnalysis[]): number {
    let totalScore = 0;
  ...
  private estimateSolveTime(size: PuzzleSize, techniques: TechniqueAnalysis[]): number {
  ...
  private getSizeMultiplier(size: PuzzleSize): number {
    switch (size) {
  ...
  private getDifficultyMultiplier(techniques: TechniqueAnalysis[]): number {
    // Calculate average complexity of techniques used
    if (techniques.length === 0) return 1.0;

    const avgComplexity = techniques.reduce((sum, t) => sum + t.complexity, 0) / techniques.length;

    // Map complexity to multiplier
    if (avgComplexity < 0.5) return 0.8; // Easy puzzles
    if (avgComplexity < 1.5) return 1.0; // Medium puzzles
    if (avgComplexity < 3.0) return 1.4; // Hard puzzles
    return 1.8; // Expert puzzles
  }

  private calculateConfidence(puzzle: Puzzle, techniques: TechniqueAnalysis[]): number {
    let confidence = 1.0;
  ...
  private applyTechnique(grid: number[][], technique: SolvingTechnique, _size: PuzzleSize): boolean {
  ...
  private applyNakedSingles(grid: number[][], gridSize: number): boolean {
    let progress = false;
  ...
  private applyHiddenSingles(grid: number[][], gridSize: number): boolean {
    let progress = false;
  ...
  private applyNakedPairs(_grid: number[][], _gridSize: number): boolean {
    // Simplified implementation - would be more sophisticated in production
    return false;
  }

  private applyHiddenPairs(_grid: number[][], _gridSize: number): boolean {
    // Simplified implementation - would be more sophisticated in production
    return false;
  }
  ...
  private isValidMove(grid: number[][], row: number, col: number, num: number, size: number): boolean {
    ...
    for (let c = 0; c < size; c++) {
      if (grid[row] && grid[row][c] === num) return false;
    }
  ...
  private getAdvancedTechniqueForDifficulty(difficulty: Difficulty): SolvingTechnique {
    switch (difficulty) {
  ...
  private addTechniquesBasedOnDifficulty(
    ...
    techniqueUsage: Map<SolvingTechnique, number>,
    emptyCells: number
  ): void {
    ...
    switch (difficulty) {
      case Difficulty.BEGINNER:
        ...
        break;
      case Difficulty.EASY:
        techniqueUsage.set(SolvingTechnique.HIDDEN_SINGLES,
          (techniqueUsage.get(SolvingTechnique.HIDDEN_SINGLES) || 0) + baseFrequency);
        break;
      case Difficulty.MEDIUM:
        techniqueUsage.set(SolvingTechnique.HIDDEN_SINGLES,
          (techniqueUsage.get(SolvingTechnique.HIDDEN_SINGLES) || 0) + baseFrequency);
        techniqueUsage.set(SolvingTechnique.NAKED_PAIRS,
          (techniqueUsage.get(SolvingTechnique.NAKED_PAIRS) || 0) + baseFrequency);
        break;
      case Difficulty.HARD:
        techniqueUsage.set(SolvingTechnique.HIDDEN_SINGLES,
          (techniqueUsage.get(SolvingTechnique.HIDDEN_SINGLES) || 0) + baseFrequency);
        techniqueUsage.set(SolvingTechnique.NAKED_PAIRS,
          (techniqueUsage.get(SolvingTechnique.NAKED_PAIRS) || 0) + baseFrequency);
        techniqueUsage.set(SolvingTechnique.HIDDEN_PAIRS,
          (techniqueUsage.get(SolvingTechnique.HIDDEN_PAIRS) || 0) + baseFrequency);
        techniqueUsage.set(SolvingTechnique.X_WING,
          (techniqueUsage.get(SolvingTechnique.X_WING) || 0) + Math.max(1, baseFrequency / 2));
        break;
      case Difficulty.EXPERT:
        techniqueUsage.set(SolvingTechnique.HIDDEN_SINGLES,
          (techniqueUsage.get(SolvingTechnique.HIDDEN_SINGLES) || 0) + baseFrequency);
        ...
        techniqueUsage.set(SolvingTechnique.HIDDEN_PAIRS,
          (techniqueUsage.get(SolvingTechnique.HIDDEN_PAIRS) || 0) + baseFrequency);
        techniqueUsage.set(SolvingTechnique.X_WING,
          (techniqueUsage.get(SolvingTechnique.X_WING) || 0) + baseFrequency);
        techniqueUsage.set(SolvingTechnique.XY_WING,
          (techniqueUsage.get(SolvingTechnique.XY_WING) || 0) + Math.max(1, baseFrequency / 2));
        techniqueUsage.set(SolvingTechnique.FORCING_CHAINS,
          (techniqueUsage.get(SolvingTechnique.FORCING_CHAINS) || 0) + Math.max(1, baseFrequency / 3));
        break;
    }
  }
}