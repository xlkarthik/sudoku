...
export class PuzzleSolver {
  ...
  private applyTechnique(grid: number[][], technique: SolvingTechnique, size: PuzzleSize): { applied: boolean; steps: SolverStep[] } {
    const steps: SolverStep[] = [];
    let applied = false;
    
    switch (technique) {
      case SolvingTechnique.NAKED_SINGLES:
        applied = this.applyNakedSingles(grid, size as number, steps);
        break;
      case SolvingTechnique.HIDDEN_SINGLES:
        applied = this.applyHiddenSingles(grid, size as number, steps);
        break;
      case SolvingTechnique.NAKED_PAIRS:
        applied = this.applyNakedPairs(grid, size as number, steps);
        break;
      case SolvingTechnique.HIDDEN_PAIRS:
        applied = this.applyHiddenPairs(grid, size as number, steps);
        break;
      default:
  ...
  private applyNakedSingles(grid: number[][], size: number, steps: SolverStep[]): boolean {
    let progress = false;
  ...
  private applyHiddenSingles(grid: number[][], size: number, steps: SolverStep[]): boolean {
    let progress = false;
  ...
  private applyNakedPairs(_grid: number[][], _size: number, _steps: SolverStep[]): boolean {
  ...
  private applyHiddenPairs(_grid: number[][], _size: number, _steps: SolverStep[]): boolean {
    // Simplified implementation for now
  ...
  private findHiddenSinglesInRow(grid: number[][], row: number, size: number, steps: SolverStep[]): boolean {
    let progress = false;
  ...
  private findHiddenSinglesInColumn(grid: number[][], col: number, size: number, steps: SolverStep[]): boolean {
    let progress = false;
  ...
  private findHiddenSinglesInBox(grid: number[][], startRow: number, startCol: number, boxSize: number, steps: SolverStep[]): boolean {
    let progress = false;
    const size = grid.length;
  ...
  private solveWithBacktracking(grid: number[][], size: number, depth: number = 0): boolean {
  ...
  private findAllSolutionsWithTimeout(
    ...
    if (solutions.length >= maxSolutions || depth > 81 || (Date.now() - startTime) > timeout) return;
    ...
    for (let num = 1; num <= size; num++) {
      if (this.isValidMove(grid, row, col, num, size)) {
        ...
        if (solutions.length >= maxSolutions || (Date.now() - startTime) > timeout) return;
      }
    }
  }

  private findConflicts(grid: number[][], row: number, col: number, value: number, size: number): ValidationError[] {
    const errors: ValidationError[] = [];
